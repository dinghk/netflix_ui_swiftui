...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let numSeason = Int.random(in: 1...3)
        var allEpisodes: [Episode] = []
        
        for i in 1...numSeason {
        ...
        let defEpiInfo = CurrentEpisodeInfo(episodeName: epiname, description: epidesc, season: 1, episdoe: 1)
        
        let allTrailers = generateTrailers(3)
        let moreLikeThis = exampleMovies
        
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, categories: categories, genre: .allGenres, year: year, rating: "PG-13", numberOfSeasons: numSeason, epidsodes: allEpisodes, promotionHeadline: headline, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creators: creators, cast: cast, moreLikeMovies: moreLikeThis, trailers: allTrailers, previewImageName: "")
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let desc = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse ata purus diam. Nullam a nulla a augue eleifend convallis ac vitae neque. Curabitur sit amet felis non libero consectetur euismod."
        let randLength = Int.random(in: 20..<59)
        
        let episode = Episode(id: id, name: name, season: season, thumbnailImageURL: thumbnail, description: desc, length: randLength, episodeNumber: num + 1, videoURL: exampleVideoURL)
